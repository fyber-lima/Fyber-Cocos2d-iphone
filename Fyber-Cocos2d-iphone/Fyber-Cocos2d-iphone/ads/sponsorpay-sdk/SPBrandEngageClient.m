...
@interface SPBrandEngageClient ()<SPBrandEngageWebViewDelegate, SKStoreProductViewControllerDelegate>
...
//    }
    ...
    [[SPConnectionHelper sharedInstance] checkConnectivityWithFailure:^{
        ...
        // Error handling
        [SPErrorManager showAlertForNoConnectionAndDismiss:^(UIAlertView *alertView, NSInteger buttonIndex) {
            self.offersRequestStatus = SPBEClientOffersRequestStatusMustQueryServerForOffers;
...
- (void)brandEngageWebView:(SPBrandEngageWebView *)BEWebView requestsToCloseFollowingOfferURL:(NSURL *)offerURL
{
    ...
    BOOL willOpenURL = [[UIApplication sharedApplication] canOpenURL:offerURL];
    if (willOpenURL) {
        [[NSNotificationCenter defaultCenter] addObserver:self
