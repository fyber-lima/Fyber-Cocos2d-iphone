...
static const CGFloat SPVideoPlayerViewControllerHintViewHeight = 30;
static const CGFloat SPVideoPlayerViewControllerHintViewDurationRatio = 0.25; // 0.25 = durtaion of hint will be 1/4 of the video length



@interface SPVideoPlayerViewController ()<SPMicroBrowserDelegate>
...
- (void)userTappedClose
{
    if (self.showAlert) {
        ...
        self.closeAlert = [SPErrorManager showWarningWithTitle:@"Exit Video" message:self.alertMessage cancelTitle:@"Resume Video" otherTitles:@"Close Video" dismiss:^(UIAlertView *alertView, NSInteger buttonIndex) {
...
- (void)movieLoadStateDidChange:(NSNotification *)notification
{
    ...
    if (self.player.loadState & MPMovieLoadStateStalled) {
        ...
        SPError *error = [[SPError alloc] initWithTitle:SPErrorAlertTitle message:SPErrorMessageRewardedVideoPlaying code:SPErrorCodeRewardedVideoPlaying];
        [SPErrorManager showAlertWithError:error dismiss:^(UIAlertView *alertView, NSInteger buttonIndex) {
            [self.player stop];
        }];
    }
}
...
- (void)willResignActive:(id)notification
{
    SPLogDebug(@"%s", __PRETTY_FUNCTION__);
    [self printPlayerState];
}

- (void)didBecomeActive:(id)notification
{
    SPLogDebug(@"%s", __PRETTY_FUNCTION__);
    ...
    if(self.player.playbackState == MPMoviePlaybackStateStopped) {
        if(self.player.isPreparedToPlay) {
            ...
            // This seems like a bug. If we don't set the URL again, the player does not load
            self.player.contentURL = self.videoURL;
            [self.player prepareToPlay];
        }
    }
...
- (void)videoStalledAtStartup
{
    ...
    SPError *error = [[SPError alloc] initWithTitle:SPErrorAlertTitle message:SPErrorMessageRewardedVideoLoading code:SPErrorCodeRewardedVideoLoading];
    [SPErrorManager showAlertWithError:error dismiss:^(UIAlertView *alertView, NSInteger buttonIndex) {
        [self videoPlaybackFinishedWithAbort:YES];
    }];
}
